...
-- After running, copy the UUID of YOUR real account and run the follow/jam sections
-- at the bottom to link yourself to the dummy users.

-- ─────────────────────────────────────────────
-- Fixed UUIDs for the 5 dummy users
-- ─────────────────────────────────────────────
-- Marco Rossi       → a1000001-aaaa-4000-8000-000000000001
-- Sofia Bianchi     → a1000001-aaaa-4000-8000-000000000002
-- Luca Ferrari      → a1000001-aaaa-4000-8000-000000000003
-- Elena Conti       → a1000001-aaaa-4000-8000-000000000004
-- Davide Russo      → a1000001-aaaa-4000-8000-000000000005
...
DECLARE
  ...
  -- highlight UUIDs
  h1a uuid := 'h1000001-aaaa-4000-8000-000000000101';
  h1b uuid := 'h1000001-aaaa-4000-8000-000000000102';
  h2a uuid := 'h1000001-aaaa-4000-8000-000000000201';
  h2b uuid := 'h1000001-aaaa-4000-8000-000000000202';
  h3a uuid := 'h1000001-aaaa-4000-8000-000000000301';
  h4a uuid := 'h1000001-aaaa-4000-8000-000000000401';
  h5a uuid := 'h1000001-aaaa-4000-8000-000000000501';

  -- jam UUID
  jam_id uuid := 'j1000001-aaaa-4000-8000-000000000001';
  jam_id2 uuid := 'j1000001-aaaa-4000-8000-000000000002';
...
VALUES
  ...
  (u2, '00000000-0000-0000-0000-000000000000',
   ...
   'authenticated', 'authenticated',
   '{"provider":"email","providers":["email"]}', '{"display_name":"Sofia Bianchi"}',
   '', '', '', ''),
  (u3, '00000000-0000-0000-0000-000000000000',
   'luca.ferrari@example.com', '',
   now(), now() - interval '20 days', now(),
   'authenticated', 'authenticated',
   '{"provider":"email","providers":["email"]}', '{"display_name":"Luca Ferrari"}',
   '', '', '', ''),
  (u4, '00000000-0000-0000-0000-000000000000',
   'elena.conti@example.com', '',
   now(), now() - interval '15 days', now(),
   'authenticated', 'authenticated',
   '{"provider":"email","providers":["email"]}', '{"display_name":"Elena Conti"}',
   '', '', '', ''),
  (u5, '00000000-0000-0000-0000-000000000000',
   'davide.russo@example.com', '',
   now(), now() - interval '10 days', now(),
   'authenticated', 'authenticated',
   '{"provider":"email","providers":["email"]}', '{"display_name":"Davide Russo"}',
   '', '', '', '')
ON CONFLICT (id) DO NOTHING;

-- ─── 2. PROFILES ──────────────────────────────────────────────────────────────
-- The trigger handle_new_user creates profiles automatically on auth insert,
-- but may not have run yet. Upsert to be safe and set display names + reading status.
...
VALUES
  ...
  (u4, 'elena.conti',  'Elena Conti',   'Il Piccolo Principe',   'Antoine de Saint-Exupéry'),
...
VALUES
  (h1a, u1, b1a,
   'I libri non sono fatti per essere creduti, ma per essere sottoposti a indagine. Di fronte a un libro non dobbiamo chiederci cosa dica ma cosa voglia dire.',
   '342', now() - interval '28 days',
   md5(u1::text || h1a::text), 'yellow'),

  (h1b, u1, b1b,
   'Se comprendere è impossibile, conoscere è necessario, perché ciò che è accaduto può ritornare.',
   '89', now() - interval '26 days',
   md5(u1::text || h1b::text), 'blue'),

  (h2a, u2, b2a,
   'La storia cominciò quando circa 70.000 anni fa avvenne una mutazione accidentale nelle catene del DNA di un Sapiens, che mutò il cablaggio interno del cervello, permettendo di pensare in modi del tutto nuovi e comunicare usando un tipo di linguaggio completamente diverso.',
   '55', now() - interval '22 days',
   md5(u2::text || h2a::text), 'orange'),

  (h2b, u2, b2b,
   'La solitudine non è stare soli. La solitudine è non riuscire a comunicare le cose che sembrano importanti.',
   '211', now() - interval '18 days',
   md5(u2::text || h2b::text), 'pink'),

  (h3a, u3, b3a,
   'La guerra è pace. La libertà è schiavitù. L ignoranza è forza.',
   '17', now() - interval '16 days',
   md5(u3::text || h3a::text), 'yellow'),

  (h4a, u4, b4a,
   'Tutte le persone grandi sono state bambine una volta. Ma poche di esse se ne ricordano.',
   '5', now() - interval '12 days',
   md5(u4::text || h4a::text), 'yellow'),

  (h5a, u5, b5a,
   'Dentro di noi c è qualcuno che sa tutto, che vuole tutto, che fa tutto meglio di noi.',
   '134', now() - interval '8 days',
   md5(u5::text || h5a::text), 'blue')
ON CONFLICT DO NOTHING;
...
VALUES
  (u1, u2, now() - interval '25 days'),
  (u1, u3, now() - interval '24 days'),
  (u2, u1, now() - interval '23 days'),
  (u2, u4, now() - interval '22 days'),
  (u3, u1, now() - interval '20 days'),
  (u3, u5, now() - interval '18 days'),
  (u4, u2, now() - interval '15 days'),
  (u5, u1, now() - interval '10 days'),
  (u5, u3, now() - interval '8 days')
ON CONFLICT DO NOTHING;

RAISE NOTICE 'Seed completato: 5 utenti, 7 libri, 7 highlights, 2 jam, follower/following inseriti.';
RAISE NOTICE 'Prossimo step: esegui la sezione sotto con il TUO user ID per collegarti agli utenti demo.';

END $$;

-- ─────────────────────────────────────────────────────────────────────────────
-- PASSO 2: Collegati agli utenti dummy (esegui separatamente)
-- Sostituisci 'IL-TUO-USER-ID' con il tuo UUID reale (trovi in Auth → Users nel dashboard).
-- ─────────────────────────────────────────────────────────────────────────────
/*
DO $$
DECLARE
  me  uuid := 'IL-TUO-USER-ID';   -- ← cambia questo
  u1  uuid := 'a1000001-aaaa-4000-8000-000000000001';
  u2  uuid := 'a1000001-aaaa-4000-8000-000000000002';
  u3  uuid := 'a1000001-aaaa-4000-8000-000000000003';
  u4  uuid := 'a1000001-aaaa-4000-8000-000000000004';
  ...
  jam_id2 uuid := 'j1000001-aaaa-4000-8000-000000000002';
BEGIN
  -- Seguimi / segui tutti i dummy users
  INSERT INTO public.follows (follower_id, following_id)
  VALUES (me, u1), (me, u2), (me, u3), (me, u4), (me, u5)
  ON CONFLICT DO NOTHING;

  -- Aggiungi te stesso ai Jam dei dummy users
